...
// 根据配置，来初始化所有的Filter，对应的是先获得对应的factory，然后通过factory的
std::vector<Network::FilterFactoryCb> ProdListenerComponentFactory::createNetworkFilterFactoryList_(
  ...
  for (ssize_t i = 0; i < filters.size(); i++) {
    ...
    Config::Utility::validateTerminalFilters(
        ...
        i == filters.size() - 1);
    // Network::FilterFactoryCb类型的callback函数
...
// 首先会创建ListenerFilter，可以对监听的socket进行定制化的配置，例如为了实现use_original_dst，就需要加入一个ListenerFilter。
...
// ListenerManagerImpl的构造函数，会根据server来创建不同的 worker
...
// Listener 配置发生变化时，会调用 LdsApiImpl::onConfigUpdate接口，然后调用该接口，创建个ListenerImpl
...
void ListenerManagerImpl::startWorkers(GuardDog& guard_dog, std::function<void()> callback) {
  ...
  for (const auto& worker : workers_) {
    ...
    // 遍历所有活跃的 listeners，将其加入到worker中
    for (const auto& listener : active_listeners_) {
