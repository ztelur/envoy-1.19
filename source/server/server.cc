...
void InstanceImpl::initialize(const Options& options,
  ...
  // 查看 MainImpl::initialize() 函数，其中是根据配置构造了静态的clustermanager和listenermanager中的listener
  ...
  if (bootstrap_.dynamic_resources().has_lds_config() ||
    ...
    // 如果bootstrap有动态配置，并且有lds配置，那么就要创建初始化的LdsApi
...
// 启动所有的 workers，也就是启动所有的Listener
